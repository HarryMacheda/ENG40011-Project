...
import json, sys, time, math, tkinter as tk
...
class PCA9548AChannel:
    ...
    def writeto(self, addr, buf, *, start=0, end=None):
        self._select(); return self._i2c.writeto(addr, buf, start=start, end=end)
    def readfrom_into(self, addr, buf, *, start=0, end=None):
        self._select(); return self._i2c.readfrom_into(addr, buf, start=start, end=end)
    def writeto_then_readfrom(self, addr, bo, bi, *, out_start=0, out_end=None, in_start=0, in_end=None):
        self._select()
        return self._i2c.writeto_then_readfrom(addr, bo, bi, out_start=out_start, out_end=out_end, in_start=in_start, in_end=in_end)

# Detect sensors on mux
MUX_ADDR = 0x70
TARGET_ADDR = 0x29
...
def detect_channels(base_bus, mux_addr=0x70, target=0x29):
    ...
    for ch in range(8):
        # select channel
        while not base_bus.try_lock(): pass
        try:
            base_bus.writeto(mux_addr, bytes([1 << ch]))
        finally:
            base_bus.unlock()

        # scan this downstream bus
        while not base_bus.try_lock(): pass
        try:
            addrs = base_bus.scan()
        finally:
...
def auto_expose(sensor, c):
    ...
    if c < C_TARGET_MIN and (it_i < len(IT_VALUES)-1 or g_i < len(GAIN_VALUES)-1):
        if g_i < len(GAIN_VALUES)-1: g_i += 1
        elif it_i < len(IT_VALUES)-1: it_i += 1
        sensor.gain = GAIN_VALUES[g_i]
        sensor.integration_time = IT_VALUES[it_i]
        return True
    return False
...
if CAL_PATH.exists():
    try: CAL_DB = json.loads(CAL_PATH.read_text())
    except Exception: CAL_DB = {}
else:
    CAL_DB = {}
...
def cal_get(ch): return CAL_DB[str(ch)]
def cal_save():
    out = {}
    for k,v in CAL_DB.items():
        vv = dict(v); vv["ccm"] = np.asarray(vv["ccm"], dtype=float).tolist()
        out[k] = vv
    CAL_PATH.write_text(json.dumps(out, indent=2))

# Color processing
def clamp8(x): return int(max(0, min(255, round(x))))
def normalize_by_clear(r,g,b,c):
    if c <= 0: return np.zeros(3)
    return np.array([r/c, g/c, b/c], dtype=float)
def apply_wb(v, wb): return v * np.array(wb, dtype=float)
def rgb_saturation(v, sat):
    m = np.array([[0.2126,0.7152,0.0722],
                  [-0.2126,0.2848,-0.0722],
                  [-0.2126,-0.7152,0.9278]])
    yuv = m @ v; yuv[1:] *= sat; invm = np.linalg.inv(m)
    return invm @ yuv
def linear_to_srgb(v, gamma):
    v = np.clip(v,0,1); return np.power(v, 1.0/float(gamma))
def srgb_to_linear(v, gamma):
    v = np.clip(v,0,1); return np.power(v, float(gamma))
def to_8bit(v): return tuple(clamp8(x*255.0) for x in np.clip(v,0,1))
...
def calibrate_ccm(sensor, ch, gamma=None, samples=12, settle=0.3):
    if gamma is None: gamma = cal_get(ch)["gamma"]
    ...
    cal["ccm"] = np.eye(3).tolist(); cal["ccm_enabled"] = False
    try:
        ...
        S = np.stack(sens, axis=0); R = np.stack(refs, axis=0)
        Sp = np.linalg.pinv(S); M_t = Sp @ R; M = M_t.T
        cal["ccm"] = M.tolist(); cal["ccm_enabled"] = True; cal_save()
        print("CCM saved.")
    finally:
...
def update_tile(ch):
    ...
    try:
        ...
        # Serial print
        print(f"Ch{ch}: RGB={R8},{G8},{B8}  HEX={hex_str}  Raw=({int(r)},{int(g)},{int(b)},{int(c)})")
...
def on_key(e):
    ...
    if lk == 'w':
        ...
        cal["ccm_enabled"] = not cal.get("ccm_enabled", True); cal_save()

    elif lk == 's':
        cal["saturation"] = max(0.10, round(cal["saturation"] - 0.05, 2)); cal_save()
    elif k == 'S':
        cal["saturation"] = min(3.00, round(cal["saturation"] + 0.05, 2)); cal_save()

    elif lk == 'g':
        cal["gamma"] = max(1.40, round(cal["gamma"] - 0.05, 2)); cal_save()
    elif k == 'G':
        cal["gamma"] = min(2.80, round(cal["gamma"] + 0.05, 2)); cal_save()

    elif lk == 'r':
        CAL_DB[str(selected_ch)] = _default_cal(); cal_save()
